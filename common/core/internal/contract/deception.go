...
type DecoyAsset struct {
	...
	Backend string
	// Hosts 是这个资产**声明拥有的主机名**（归属声明，`W7`）。
	//
	// 为什么路径归属必须带主体：只按 Path 的话，「我拥有 /admin」这句话对**所有**主机成立 ——
	// 真实站点本来就有 /admin 时，我们会在没有任何声明的情况下把它的路径接走。
	// 带 Host 之后，声明变成「我拥有 admin.example.com 上的 /admin」，冲突面收敛到明确的主机。
	//
	// 形态：精确主机名（`console.example.com`）或前缀通配（`*.example.com`）。
	// 端口不参与匹配（比较时剥离）；大小写不敏感（存储时统一小写）。
	Hosts   []string
	Enabled bool
}
