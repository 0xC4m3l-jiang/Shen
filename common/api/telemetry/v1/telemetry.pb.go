...
//
//  1. 已经**允许离开核心**的（策略版本 / 校验和 / AI 配置，它们本就在 `common/api/policy/v1` 的载荷里）；
//  2. 比前者**更弱**的描述性信息（如白名单**条数** —— 载荷里本来就有全量 CIDR 列表）。
//
// 因此**不**包含：阈值 / 灰度 / 误调度预算 / 影子模式 ——
// 它们是核心运行参数，`common/core/internal/contract/thresholds.go` 明确禁止写进 `common/api/*.proto` 的对外响应
...
const file_telemetry_v1_telemetry_proto_rawDesc = "" +
	...
	"\vWatchEvents\x12 .telemetry.v1.WatchEventsRequest\x1a\x18.telemetry.v1.WatchEvent0\x01B*Z(shen/common/api/telemetry/v1;telemetryv1b\x06proto3"
