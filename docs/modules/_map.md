...
| [`deception/`](../../deception) | **① 欺骗层**：L1 四个接入形态的适配器与处置 + L3 网络欺骗声明式产物 | Go / 配置 / 声明式 | 是 |
| [`honeypot/`](../../honeypot) | **② AI 蜜罐层**：蜜罐协议仿真入口（`protocol/`）+ 假 shell（`shell/`，**未建**） | Go / Rust | 是 |
...
| --- | --- | --- |
| ① **欺骗层**（反向代理 + 欺骗内容注入） | `deception/` 四个适配器与改写（`proxy` ③前置 + ④Sidecar · `mirror` ①镜像 · `dns` ②引流 · `injection` 响应改写）· `analysis/aicap/`（生成欺骗内容）· `deception/netpolicy/`（网络层欺骗：假拓扑 / 微隔离） | `judge`（判定）· `director`（三值决策）· `responder`（欺骗响应生成）· `session` · `isolation` · `policy`（版本与下发）· `decoy`（诱饵面） |
| ② **AI 蜜罐层**（入口 + 关联不同蜜罐） | `honeypot/protocol/`（协议仿真框架）· `honeypot/shell/`（假 shell，**未建**） | `honeypot`（**入口与后端池**：代码在核心、功能属本层）· `analysis/` 的 `intent` · `chain` · `strategy` · `llm` 与 worker（蜜罐「聪明的那一半」） |
...
| `deception/` | Go | **内嵌 Caddy**（`github.com/caddyserver/caddy/v2`，Apache-2.0；**传递依赖树很大**——上一行的 `go list` 输出几乎全是它的依赖）· gRPC · protobuf | 转发与 TLS 终结**复用** Caddy（[ADR-0017](../background/decisions/0017-caddy-l1-base.md)）；本层只写判定胶水 |
...
| 只跑某一层 | `go test ./core/...` · `go test ./deception/...` · 在 `analysis/` 里跑 `.venv/bin/pytest` |
...
| 9 | `edge-injection` | [`deception/injection/`](../../deception/injection) | 投毒响应改写 · 假路径 · 蜜饵注入（**不独立部署**，`ST-5`） | `Injector` · `Rule` | 纯变换（连 `contract` 都不依赖）→ `deception/proxy` |
| 10 | `adapter-mirror` | [`deception/mirror/`](../../deception/mirror) | ① 旁路镜像接收端：只读采集、**不在请求路径**（`INT-6`） | `Receiver`（HTTP） | `api/judge/v1` · `api/telemetry/v1` |
| 13 | `adapter-dns` | [`deception/dns/`](../../deception/dns) | ② DNS 引流：按来源解析到引擎或真实服务（**纯配置，无源码**，`AR-3`） | `config/Corefile.example` | CoreDNS（复用） |
| 11·12 | `adapter-proxy` | [`deception/proxy/`](../../deception/proxy) | ③ 反向代理前置 + ④ Sidecar（**同一份实现**）：白名单 → 判定缓存 → 调核心 → 三值处置 → 异步上报；转发与 TLS 用内嵌 Caddy（`ADR-0017`） | `JudgeClient` · `TelemetryClient` · `Injector` · `Config` | `api/*` · `deception/injection` |
...
| # | 模块 | 目录 | 能力 | 对外接口 | 接线 |
| --- | --- | --- | --- | --- | --- |
| 14 | `honeypot-protocol` | [`honeypot/protocol/`](../../honeypot/protocol) | 协议仿真**框架**：协议注册表 · 会话工厂 · 运行框架（并发上限 `MD-16` · 对称回收 `MD-15`）· 最小真实适配器 | `Protocol` · `Session` · `SessionFactory` · `Registry` · `Limits` · `Stats` | 独立于核心；真实协议栈内容待专项调研 |
| 15 | `honeypot-shell` | ⏸ **未建**（[`honeypot/shell/`](../../honeypot) 不存在） | 命令表分发 · 内存文件系统 · 文件投递 | — | 用户裁定推迟（蜜罐只做接入架构） |
...
| 25 | `ai-capability` | [`analysis/aicap/`](../../analysis/aicap) | **可开关的生成出口**：任务注册表（缺护栏档案即启动失败）· 前置提示词三段式（`AR-31`/`AR-24`）· 后置四关校验（`AR-15`/`AR-22`/`AR-23`/风格）· 内容对象与清单（`content_id` 由内容体算出：**产物层**确定性，不是生成期要求） | `generate(TaskSpec) → Envelope` · `python -m analysis.aicap` | 清单文件 → 核心 [`policy`](../../core/internal/policy) 装载 → 策略载荷 → [适配器](../../deception/proxy) 注入（`AR-33` / [ADR-0023](../background/decisions/0023-deception-content-injection.md)） |
...
| `shen-proxy` | [`deception/proxy/cmd/proxy/main.go`](../../deception/proxy/cmd/proxy/main.go) | ③ 前置 + ④ 边车（内嵌 Caddy 处理转发与 TLS） |
| `shen-mirror` | [`deception/mirror/cmd/mirror/main.go`](../../deception/mirror/cmd/mirror/main.go) | ① 旁路镜像接收端（恒回 202，不在请求路径） |
...
| 接缝 | 从 → 到 | 契约 | 说明 |
| --- | --- | --- | --- |
| **S1 判定** | `deception/*` → `core` | [`api/judge/v1`](../../api/judge/v1) `DeceptionJudge.Judge` | 适配器把观测发给核心，拿回三值决策；deadline 受延迟预算约束（`AR-29`） |
| **遥测上报** | `deception/*` · L4 → `core` | [`api/telemetry/v1`](../../api/telemetry/v1) `Report` / `ReportBatch` | 异步、批量、幂等（`AR-11`，幂等键 `event_id`） |
| **观测读取** | 控制台 · L4 → `core` | `api/telemetry/v1` `ListEvents` | 读侧：控制台看告警/流量，L4 拿输入（[`../spec/events.md`](../spec/events.md)） |
| **S4 策略面** | `core` → `deception/*` | [`api/policy/v1`](../../api/policy/v1) `Pull` / `Ack`（`Watch` 未实现） | 拉取式：后端表 · 白名单 · 注入规则；带版本 + 校验和 + 回执（`ST-8` · `AR-13` · [`../spec/policy-payload.md`](../spec/policy-payload.md)） |
| **注入** | `deception/proxy` → `deception/injection` | Go 接口 `Injector` | 进程内引用，**不独立部署**（`ST-5`） |
...
```text
客户端 → L0（客户 LB，终结 TLS）→ deception/proxy
...
| 适配器环境变量 | [`deploy/docker/README.md`](../../deploy/docker/README.md) · [`deception/proxy/config/front-proxy.example.env`](../../deception/proxy/config/front-proxy.example.env) | 各适配器进程 |
| 代理侧默认配置模板 | [`deception/proxy/config/sidecar.example.yaml`](../../deception/proxy/config/sidecar.example.yaml) | 形态 ④ 部署时 |
...
| 要件 | 说明 | 规则 |
| --- | --- | --- |
| 目录 | `core/internal/<模块>/`（核心）· `deception/<模块>/`（适配器）· `analysis/<模块>/`（L4） | `MD-1` |
