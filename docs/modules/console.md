...
**做什么**（⬆ = **设计职责，尚未实现**：当前实现是**只读观测**，见 §2 与状态行）：

- ⬆ **策略编排**：查看 / 编辑 / 发布策略（经策略平面，`ST-8`）；
- ⬆ **诱饵资产管理**：查看 / 投放 / 轮换 `decoy` 的诱饵资产；
- ⬆ **蜜罐管理**：查看 `honeypot` 的后端池与类型开关（`config` 驱动的只读视图 + 开关）；
- ⬆ **人工干预**：白名单、隔离名单的**解除**（只读查看已有：白名单只给**条数**）；
- ✅ **审计查询（只读部分）**：攻击流量、会话、攻击链、执行历史 —— 由逐判定日志 / 逐请求链路 / L4 结论 / 原始事件四块承担；
- ✅ **运行态观测**（本轮与上两轮新增）：核心当前配置快照 · **观测新鲜度**（数据停没停）。
