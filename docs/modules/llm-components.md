...
| 类型 | 覆盖什么 | 位置 |
| --- | --- | --- |
| 单元 | 契约校验（喂坏输出**必须**失败） | `analysis/llm/`（待建） |
| 单元 | JSON 三段式解析 + 超长输出 | 同上 |
| 单元 | 黑名单三类各一例 | 同上 |
| **注入样本回归** | 攻击者内容含指令时，**结论不得改变**（`AR-31`） | 同上（与 `MD-7` 正负样本同构） |
| 超时注入 | 双阶段收尾 + 整体作废 | `AR-19` / `AR-21` |
...
| 3 | `strategy` 生成策略时的注入面 | 策略链路安全 | `strategy.md`（待建） |
