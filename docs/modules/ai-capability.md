...
| 阶段 A 的确定性模板生成器（`tasks/content.py`） | `Faker`（MIT，19.4k★）· `Mimesis`（MIT） | 两者都有 `seed()` ⇒ 能保住「生成期可复现」（**阶段 A 的工程性质**，不是 `AR-30` 的要求）；但落地属阶段 B（与真实模型后端一起做），且需先验 `seed()` 的跨版本稳定性（ADR-0024 失效条件 5） |
