...
| 负责人 | —— |
| 状态 | 阶段 A 已实现（通路 · 开关 · 强制护栏）；风格画像属阶段 B；**三个 L4 任务（`intent` / `chain` / `strategy`）已登记为 kind**，模型后端已接（`llm/deepseek.py`，`--llm` 才走） |
| 最后更新 | 2026-09-21 |
...
> （7 个能力逐个详解 · 模型后端硬约束 · 优化点候选（见该文 §8）· 6 条关键发现 · **§12 模块生命周期** · **§13 怎么接入被使用**）。
...
| 阶段 A 的确定性模板生成器（`tasks/content.py`） | `Faker`（MIT，19.4k★）· `Mimesis`（MIT） | 两者都有 `seed()` ⇒ 能保住「生成期可复现」（**阶段 A 的工程性质**，不是 `AR-30` 的要求）；但落地仍属阶段 B（假数据生成；模型后端已于 2026-09-21 接通，两件事不再必须一起做），且需先验 `seed()` 的跨版本稳定性（ADR-0024 失效条件 5） |

## 4. 关键规则

| 规则 | 与本模块的关系 |
| --- | --- |
| `AR-33` | **本模块存在的理由**：**任何** LLM 生成必须经本模块的唯一出口并强制过护栏（口径 2026-09-20 由「欺骗内容」放宽，[ADR-0025](../background/decisions/0025-generic-guardrailed-outlet.md)）；模型客户端只允许被本能力自身 import（`service.py` 出口 · `model.py` 接缝；能力之外一律禁止——门禁结构检查） |
...
| `AR-30` | 热路径的字节一致：**产物冻结进清单** + `content_id` 由**内容体**算出 + **会话钉定**。⚠️ 生成期的「同输入逐字节同输出」是**阶段 A 的工程性质**，**不是** `AR-30` 的要求（它只管响应路径）；接模型后生成不可复现（实测），见 [ADR-0026](../background/decisions/0026-cloud-model-backend.md) 决定 2 |
