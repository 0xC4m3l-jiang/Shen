...
> **权威清单在 [`../design/modules.md`](../design/modules.md) §1.1**（**23 个有效模块**，共 24 行；第 12 行 `adapter-sidecar` 已合并）。
...
> 编号不按位置递增（`control` 编号 22、`decoy` 23、`honeypot` 24）—— 编号是引用句柄，保持稳定（`MD-18` / `D-6` 精神）。
...
请求 → 白名单（INT-25）→ 本地判定缓存（AR-6）→ core.Judge → 按三值处置
         ...
         ├─ route_mirage → 查**适配器自己的**引流表 → 注入诱饵（INT-8）→ 后端
         └─ block        → 403（默认不产出，Q5）
```
...
| 模块 | 现状 | 为什么到不了边缘 |
| --- | --- | --- |
| `decoy` | 仅启动装配 + 日志 | 诱饵定义要到边缘才有用，而**策略平面未实现** |
| `responder` | 仅启动一致性子检 + 装配 | 同上；且 `ST-3` 使边缘**不能** import 核心内部包 |
| `honeypot` | 仅启动装配 + 日志 | 适配器用**自己的** `SHEN_PROXY_MIRAGE` 解析后端名 —— **两份事实源** |

> ⭐ **当前欺骗引擎的唯一断点**：`api/policy/v1` 已定义策略面（`Pull` / `Watch` / `Ack`，接缝 **S4**），
> 但**核心侧未实现服务端、边缘侧未实现客户端** —— 于是「核心算出的诱饵 / 后端池 / 响应内容」传不到边缘，
> 边缘只能靠**自己的环境变量**工作（`SHEN_PROXY_MIRAGE` / `SHEN_PROXY_INJECT`）。
> **补齐它 = 欺骗引擎的闭环闭合**（见 [`../design/structure.md`](../design/structure.md) §2.1）。
