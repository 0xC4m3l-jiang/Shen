...
2. `make ai-check`（模板）**21/21**、`make ai-check-llm`（真模型）**23/23**，且 **AI 模式**下断言「清单 `generator=model-v1`」、
...
```console
$ SHEN_AI_KEY=… python3 scripts/dev/ai-inject-check.py --llm --block --keep --dag-out /tmp/dag
✅ 全部通过（23 项）
...
| 15 | 报告里的证据快照是**加硬 harness 之前**那次运行（22 项），而现在的 harness 是 23 项 | 报告与工具版本错位（同类问题的第二次） | 我加了阶段① 时序重试后项数变化 | 修正 | ✅ 用**终版 harness** 重跑并整体重取证据（23 项），报告重新合成；此后 §6 引文与新 `check-log.txt` 逐字一致 |
...
| 2026-09-21 | **评审后修正（第二次证据快照）**：用**终版 harness**（23 项）重跑并重取全部证据，报告重新合成，§6 引文与 `check-log.txt` 逐字一致 | 报告与工具版本必须同源（§7.1 #15） |
