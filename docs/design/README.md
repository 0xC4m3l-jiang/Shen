...
> 规则总数 **153 条**：`AR` 29 · `INT` 25 · `ST` 24 · `MD` 24 · `NI` 14 · `TB` 12 · `TM` 11 · `SB` 8 · `OH` 5 · `BA` 1；
> 另有本文件的 `D-1…D-8`。
>
> 确认轨迹：
> · **C1–C6 轮**确认 144 条
...
| 4 | [`modules.md`](modules.md) | **有哪些模块、各自能干什么** —— **§1.1 权威模块清单**（22 行 / 21 个有效模块）、职责、边界、依赖方向、阶段、失败模式 | `MD` |
