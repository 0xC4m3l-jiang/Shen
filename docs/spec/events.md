...
**夹具（唯一事实源）**：[`../../common/api/telemetry/v1/testdata/decision_event.json`](../../common/api/telemetry/v1/testdata/decision_event.json) —— 由 Go 结构体直接生成。
...
2. 重新生成夹具 `common/api/telemetry/v1/testdata/decision_event.json`；
3. 两侧契约测试同步：Go 侧 `common/core/internal/control/observer_contract_test.go`、Python 侧 `analysis/tests/test_event_contract.py`。
...
`inject` 的四个取值（**唯一权威定义**，改它必须同步 `modules/deception/proxy` 常量与本文）：
...
`executed` 的七个取值（**唯一权威定义**，改它必须同步 `modules/deception/proxy` 的 `executedFor` 与本文）：
...
**夹具**：[`../../common/api/telemetry/v1/testdata/request_judged_event.json`](../../common/api/telemetry/v1/testdata/request_judged_event.json)（由 `modules/deception/proxy` 的契约测试逐键比对）。
