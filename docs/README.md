...
>
> 想先知道「它是什么 / 怎么启动」→ 看根 [`README.md`](../README.md)。（本轮起根 README 只放**项目介绍 + 项目启动**；效果演示、目录结构、门禁纪律、已知限制等信息都在本目录内。）
...
**还未接通的**：诱饵资产到边缘的通路（诱饵面目前 observe-only）；真实**高交互**蜜罐的内容（只做接入架构，内容待专项调研）。
**已接通（2026-09-21）**：**模型后端** —— L4 三任务（意图/攻击链/策略）与欺骗内容（`kind=content`）都能走模型
（`--llm` 显式开启，默认关；失败回落模板并如实标注生成器）。功能验证报告（含 DAG 图）：
...
**四层视图**（功能视角；入口 [`../README.md`](../README.md) §1 有同一视角的 mermaid 图，这里是文字版）：

```text
欺骗层          L1 适配器 + 核心：判定 judge → 决策 director（三值）→ 按值执行
                ★ 判定与响应生成只在这里实现一次（AR-2 / AR-5）
                （核心是欺骗层与蜜罐层**共用**的内核）
蜜罐层          L2 协议仿真（modules/honeypot）+ 核心幻境入口与后端池 + 第三方真蜜罐
web 真实业务层   被保护的真实业务站（放行目标，原样透传 INT-8；引擎挂了它照常服务 NI-1）
                ※ **不是我们的层** —— 它是被保护对象
AI 模型层       L4（analysis/）：离线生成内容 → 清单 → 策略面 → 欺骗层注入；
                近线 worker 读遥测出结论（不碰响应路径）；模型后端 = 云模型（DeepSeek）
                ※ analysis/aicap 两处都沾：功能上服务欺骗层，实现上属本层
（不在本四层图里：L3 网络欺骗 netpolicy · 管控平台 = Console 只读观测台；
 另有 L0 接入 = 复用现成组件不自研）
```

> ⚠️ **编号提醒**：`docs/modules/` 与 `modules/README.md` 里那套 circled 编号（**① 欺骗层 / ② AI 蜜罐层 / ③ 管控平台**）
> 是**三个大模块**的编号，与本节的四层**不是同一套**。

规范性分层与 24 个模块的编号（供查代码用）：
