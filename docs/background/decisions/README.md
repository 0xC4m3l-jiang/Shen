...
| [0029](0029-three-module-dirs.md) | **顶层目录与三个大模块对齐**：`edge/` → `deception/`（① 欺骗层）· `deception/honeypot/` → `honeypot/protocol/`（② AI 蜜罐层）· `console/`（③ 管控平台）；`core/`（共享内核）与 `analysis/`（跨层）**不改名** | ✅ **已采纳并执行** | `ST-1` · `MD-21` · [`design/structure.md`](../../design/structure.md) §1.1 · [ADR-0007](0007-repo-layout.md)（**部分被本文取代**） | 4 项 |
