...
| [0025](0025-generic-guardrailed-outlet.md) | **AI 生成出口解耦**：内核任务无关化（不认识「内容」）+ 消费方登记 + 产物交给 `Sink` 缝隙 +「独立」由门禁 `MD-4` 保证；并让 `checked_fields` / `max_output` 两个空声明真的生效 | ✅ **已采纳**（决定 1–3）· 🟡 **提案中**（决定 4：`AR-33` 措辞放宽为任何生成，待用户确认） | `AR-15` `AR-22` `AR-23` `AR-33` `MD-4` · [`spec/ai-contract.md`](../../spec/ai-contract.md) · [`plans/2026-09-20-aicap-decoupling.md`](../../plans/2026-09-20-aicap-decoupling.md) | 5 项（含「`AR-33` 是否放宽」待用户裁定） |
