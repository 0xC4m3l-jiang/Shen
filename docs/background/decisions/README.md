...
| [0024](0024-ai-oss-reuse-boundary.md) | **AI 能力层的开源复用边界**：按能力分层复用（判据 = 默认行为是否 fail-closed，不看功能表）+ **补上 Python 依赖审计面**（`TB-16` 的实现缺口）+ 本轮只判定不改 AI 代码 | ✅ **已采纳** · 🟢 **4 项未解决已由 [0031](0031-analysis-reuse-and-model-backend.md) 收口** | `AR-15` `AR-21` `AR-22` `AR-30` `AR-33` · `TB-16` · [`spec/dependencies.md`](../../spec/dependencies.md) · [`research/ai-oss-reuse.md`](../research/ai-oss-reuse.md) | 4 项 → 已收口（见 0031） |
...
| [0027](0027-observability-push.md) | **观测面推送**：核心事件流（gRPC 服务端流 `WatchEvents`）+ 控制台 SSE —— 记到即推，不再轮询；三条纪律（**旁路** · **无背压且丢包可见** · **可补漏**）；**策略面仍 Pull**（与 [0018](0018-policy-plane-pull-model.md) 的边界写清） | ✅ **已采纳**（2026-09-21 用户确认） | `AR-6` `AR-10` `NI-1` `MD-10` · [`spec/console-api.md`](../../spec/console-api.md) §3 · [0018](0018-policy-plane-pull-model.md)（边界） | 4 项 |
| [0028](0028-three-module-view.md) | **三个大模块（功能视角）↔ 五个平面（代码视角）**：`core/` 是三者**共用的内核**（不拆散）—— 保住 `AR-2`/`AR-5` 的**结构支撑**；蜜罐「启动」先按**路由**落地、**编排**另开 ADR；管控平台**保持只读** | ✅ **已采纳**（决定 1–6；目录重排已由 [0029](0029-three-module-dirs.md) 执行） | `AR-2` `AR-5` `TB-22` `TM-3` · [ADR-0007](0007-repo-layout.md) · [`modules/_map.md`](../../modules/_map.md) §1.1/§1.2 | 5 项 |
| [0029](0029-three-module-dirs.md) | **顶层目录与三个大模块对齐**：`edge/` → `deception/`（① 欺骗层）· `deception/honeypot/` → `honeypot/protocol/`（② AI 蜜罐层）· `console/`（③ 管控平台）；`core/`（共享内核）与 `analysis/`（跨层）**不改名** | ✅ **已采纳并执行** · 🟡 **顶层布局已被 [0030](0030-two-level-layout.md) 取代**（名字保留） | `ST-1` · `MD-21` · [`design/structure.md`](../../design/structure.md) §1.1 · [ADR-0007](0007-repo-layout.md)（**部分被本文取代**） | 4 项 |
| [0030](0030-two-level-layout.md) | **顶层收成两级**：`modules/{deception,honeypot,console}`（产品功能模块，**开发从这里进**）+ `common/{core,api}`（公用代码）；`analysis/` 留顶层（跨层 + 另一种语言） | ✅ **已采纳并执行** | `ST-1` / `ST-2` · [ADR-0029](0029-three-module-dirs.md)（**部分被取代**）· [ADR-0004](0004-terminology.md)（项目名未定）· [ADR-0028](0028-three-module-view.md) 决定 1 | 3 项 |
| [0031](0031-analysis-reuse-and-model-backend.md) | **L4 的复用边界与模型后端口径**：收口 [0024](0024-ai-oss-reuse-boundary.md) 的四项未解决 —— 云模型 + 现有三段式提取（不引 Outlines/Instructor）· 本轮不引 Presidio（改为提示词禁令 + 受检字段）· Faker/Mimesis 阶段 B 再选 · L4 **零新增依赖**（⛔×3 / 🟡×2）· 三个 L4 任务登记为 kind 并经唯一出口 + 护欏 | ✅ **已采纳**（2026-09-21 用户确认） | `AR-15` `AR-16` `AR-17` `AR-22` `AR-24` `AR-30` `AR-31` `AR-32` `AR-33` · `INT-11` · `NI-1` · [`research/l4-oss-reuse.md`](../research/l4-oss-reuse.md) | 4 项 |
