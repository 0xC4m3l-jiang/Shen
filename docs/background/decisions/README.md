...
| [0028](0028-three-module-view.md) | **三个大模块（功能视角）↔ 五个平面（代码视角）**：`core/` 是三者**共用的内核**（不拆散）—— 保住 `AR-2`/`AR-5` 的机器判据；蜜罐「启动」先按**路由**落地、**编排**另开 ADR；管控平台**保持只读**；本轮**不搬目录**（先方案后搬） | ✅ **已采纳**（决定 1–6）· 🟡 **目录重排方案待确认** | `AR-2` `AR-5` `TB-22` `TM-3` · [ADR-0007](0007-repo-layout.md) · [`modules/_map.md`](../../modules/_map.md) §1.1/§1.2 | 5 项 |
