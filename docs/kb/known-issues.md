...
> 最后更新：2026-09-20（`K-25` / `K-26` 来自 Python 依赖许可审计面的补齐，`K-27` 来自变更日志的路径核查；`K-9`…`K-11` 来自 `E1` spike；`K-12`…`K-15` 来自架构门禁的建设；`K-16`/`K-17` 来自四个接入形态的落地；`K-18` 来自装配开发循环；`K-19` 来自 `policy` 模块的落地；`K-20` 来自策略面端到端验证）

---

## 分类（现行 / 历史）

**现行**（今天仍会撞上）：`K-1` `K-2` `K-3` `K-4` `K-5` `K-6` `K-7` `K-8` · `K-12` `K-13` · `K-14` `K-15` `K-16` · `K-17` `K-18` `K-19` `K-20` · `K-21` `K-22` `K-23` `K-24` · `K-26` `K-27`

（`K-25` 已修复：Python 依赖的许可审计面已于 2026-09-20 补上 —— 保留它是因为**那类盲区仍可能复发**。）
...
## K-25 · 依赖许可审计曾经**只覆盖 Go**：Python 依赖是个空门窗

**症状**：`docs/spec/dependencies.md` 里看不到 `grpcio` / `protobuf` / `PyYAML`，
而 `analysis/requirements.txt` 明明声明了它们。`TB-16` 写着「依赖**必须**经许可与漏洞审计」——
那句话当时只对一半语言成立。

**原因**：`scripts/licensecheck` 用 `go list -deps` 反推要审的模块，**看不见 Python 依赖**。
后果不是「少一张表」，而是 [ADR-0023](../background/decisions/0023-deception-content-injection.md)
的未解决 1/2（模型后端、PII 检测）卡在「先过许可证台账」上，而台账根本无法回答那个问题。

**结论**：已于 2026-09-20 补上（[ADR-0024](../background/decisions/0024-ai-oss-reuse-boundary.md) 决定 2）：
依赖集合取 `analysis/requirements.txt` 的**运行期传递闭包**，许可声明取已安装发行版的 `*.dist-info/METADATA`。
**同类陷阱**：任何「按依赖枚举工具的输出」建立的审计都有盲区，加语言时一定要问「这个工具看不看得见新语言」。

---

## K-26 · 改了 `requirements.txt` 的锁版本就必须重跑 `make pyenv`，否则门禁会红

**症状**：`make gate` 报

```console
  ✗ protobuf                                             未审               需人工判定
      锁文件 7.36.2 vs 环境 7.35.1 不一致 —— 先跑 make pyenv（禁止用 A 版本的元数据审定 B 版本的许可）
```

**原因**：Python 侧的许可审计**故意**把「锁文件声明的版本」与「环境实际装的版本」对比 ——
从环境读到的元数据只能证明**那个版本**的许可，拿它去审另一个版本等于白审。
所以不一致时**必须失败**，而不是静默按环境版本放行。
本仓库真撞到过一次：`protobuf` 锁在 `7.36.2`、环境里还是 `7.35.1`（`pip freeze` 实测）。

**结论**：**改了 `analysis/requirements.txt` / `requirements-dev.txt` 就跑 `make pyenv`**。
这不是坑，是那道检查在干活 —— 它把「环境与声明不一致」变成了可见的失败，而不是看不见的威胁面。

---

## K-27 · 变更日志里反引号包的任何「含斜杠的串」都会被当成路径核

**症状**：`make trace` 报

```console
  ✗ DEV-2  最新条目引用的路径不存在（日志与代码已经漂移）
        analysis/intent|chain|strategy
```

而你写的明明是句简写或相对路径（如 `kb/capabilities.md` 而不是 `docs/kb/capabilities.md`）。

**原因**：`scripts/tracecheck/main.go` 的 `backtickPaths` 把**变更日志最新条目**里所有反引号内的、
**含 `/` 的**串都当成路径去核存在性 —— 它不区分「这是路径」还是「这是简写/仓库名/包名」。
**踩过两次**：上一轮是 `protectai/llm-guard` 这种 `owner/repo`，这一轮是 `analysis/intent|chain|strategy` 与 `llm/prompts.py`。

**结论**：写日志条目时：

1. 反引号里只写**仓库存根的完整相对路径**（目录带尾斜杠也算）；
2. 包名 / 仓库名 / 简写**不要放反引号**，或者不要带 `/`（如写成「`analysis/intent` · `analysis/chain`」分开写）；
3. 命令输出块（围栏代码）里的路径**不受检查** —— 报错信息可以照原件贴。

---

## K-28 · `make pygen` 永远有 diff：生成物被格式化过，而下一次生成又把它复原

**症状**：`make pygen` 每次都报「改写导入 1 个文件」，`git status` 里 `analysis/proto/**` 又变了；
重新生成后 `make gate` 的 `ruff format --check` 红。反复提交也治不好。

**原因**（两条叠在一起）：

1. `analysis/pyproject.toml` 的 `[tool.ruff]` 有 `extend-exclude = [".venv", "proto"]`，
   但那**只对目录遍历生效**：编辑器 / 预提交 / 各种「只检查我改过的文件」的工具会**显式传路径**，
   而 ruff 默认不因 `extend-exclude` 跳过**显式传入**的文件 —— 要跳过必须设 `force-exclude = true`。
   于是 `analysis/proto/` 被某一次显式调用格式化过（双引号 + black 风格），
   而 `grpc_tools.protoc` 产出的是原始形态（单引号、未格式化）。
2. 于是「提交现状」只能治一次：下次 `make pygen` 照样恢复原始形态。

**结论**（2026-09-21 修）：`[tool.ruff]` 加 **`force-exclude = true`**，
并**接受一次**生成器的原始形态入库。验收判据是可复算的：**连跑两次 `make pygen`，字节零 diff**
（实测：两次生成的 `md5` 相同；`import warnings` 这类「看起来没用」的生成物行也不再被自动删除）。

**同类陷阱**：任何「排除规则」都要问一句 _它对显式传路径的调用有效吗_。
排除规则写对了、但被调用方式绕过 —— 那种检查会看起来在工作，实际拦不住。

---

## K-29 · 演示业务站是单线程的：一条长连接会把**所有**其它请求挂住

**症状**：改道到「幻境后端」的请求 **10~20 秒无响应**（适配器日志 `落点=mirage … 字节=0 耗时=10112.1ms`），
看起来像「引擎或幻境后端坏了」；但直连业务站、或走 `route_origin` 的请求却正常。

**原因**：`scripts/demo/business.py`（演示用的假业务站）用的是单线程 `HTTPServer`，
却声明了 `protocol_version = "HTTP/1.1"`（长连接）。适配器（Go 的 `http.Transport`）**保持连接**，
于是这个站**唯一的 worker 被那一条连接长期占着** —— 其它任何连接（包括幻境那一跳）都排不进去。
`/proc/net/tcp` 里端口是 LISTEN、`rx_queue` 为 0，很容易被误读成「服务是好的、是引擎的问题」。

**结论**（2026-09-21 已修）：演示站改成 `ThreadingHTTPServer`（标准库，一行）。
它是**演示物料**、不是产品代码，但要让它像真业务站一样能并发 —— 否则「改道是否生效」这类结论不可信。

**同类陷阱**：任何自写的测试替身（假后端 / 假站点）只要**既长连接又单线程**，就会把并发测试变成随机红灯。
选替身时先问一句：**它能不能同时服务两条连接？**

---

## H-1 · 已移除的技能（历史：为什么删 · 怎么恢复）
