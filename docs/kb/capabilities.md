...
> 最近一次审计：2026-09-21（逐项取证据，非印象）。
>
> **关于计数**：本文**不再写死易漂的数字**（测试函数数 / 行数）——它们每轮都变，写死必漂。
> 要准确值请用「怎么验」那一列的命令自己取（如 `wc -l`、`go test`）。
...
| 项 | 内容 |
| --- | --- |
| 代码 | [`modules/deception/proxy/`](../../modules/deception/proxy)（想看规模与用例数：`go test ./modules/deception/proxy/ -v`） |
...
| 形态覆盖 | ③ 反向代理前置 与 ④ Sidecar 是**同一份实现**；① 旁路镜像在 [`modules/deception/mirror/`](../../modules/deception/mirror)；② DNS 引流是**纯配置**（[`modules/deception/dns/`](../../modules/deception/dns)） |
| 怎么验 | `scripts/shen.sh up` → 开控制台看 DAG；`make docker-log S=proxy`（看「路由：… 落点=…」）；`go test ./modules/deception/proxy/` |
| 边界已锁 | WebSocket 101 · SSE 不缓冲 · 2 MiB 响应不注入 · 8 MiB 上传不丢字节（见 [`forwarding_test.go`](../../modules/deception/proxy/forwarding_test.go)） |
...
| 项 | 内容 |
| --- | --- |
| 核心配置 | [`deploy/config/config.example.yaml`](../../deploy/config/config.example.yaml) + 字段全表与约束 [`../spec/config.md`](../spec/config.md)（行数用 `wc -l` 取，不写死） |
| 校验 | **严格拒绝未知键**；示例配置有**漂移守卫单测**（`common/core/internal/policy` 的 `TestExampleConfigLoads`）；`make check-config` 可干跑 |
| 策略面（S4） | 改道后端表 / 白名单 / 注入规则经 `Pull` + `Ack` 下发（`ADR-0018`），载荷契约 [`../spec/policy-payload.md`](../spec/policy-payload.md)；远端优先、本地兜底 |
| 适配器/控制台/L4 | 适配器的全量环境变量示例：[`../../modules/deception/proxy/config/front-proxy.example.env`](../../modules/deception/proxy/config/front-proxy.example.env)（含 `SHEN_PROXY_INJECT_CONTENT`）；部署入口的变量在 [`../../deploy/docker/README.md`](../../deploy/docker/README.md) 与 [`deploy/docker/compose.yaml`](../../deploy/docker/compose.yaml)；本地进程起法见 [`../ops/runbook.md`](../ops/runbook.md) §1.2 |
...
### 1.5 注入 AI 欺骗信息 🟡 **分阶段**（阶段 A 通路已通：生成 → 护栏 → 下发 → 注入；`kind=content` 接模型属阶段 B）

> 📖 **要看「AI 能力到底有哪些、模型接没接、哪里要优化」** → 本文姊妹篇 [`ai-capabilities.md`](ai-capabilities.md)
> （7 个能力逐个详解 · 模型后端 8 条硬约束 · 优化点候选（见该文 §8）· 6 条关键发现 · **§12 模块生命周期 · §13 接入与使用**）。本节只讲**状态与怎么验**。

| 层 | 状态 | 说明 |
| --- | --- | --- |
| **注入机制** | ✅ 已实现 | [`modules/deception/injection/`](../../modules/deception/injection)：只改**改道侧** HTML 响应（`INT-8`），按 `marker` 定位、找不到就跳过（不阻断响应）；规则由策略面 `inject_rules` 下发（`injects[]` 为 `[]` 表示显式关闭） |
| **AI 能力服务**（生成出口） | ✅ **已实现（阶段 A）** | [`analysis/aicap/`](../../analysis/aicap)：唯一出口 `generate(TaskSpec) → Envelope`，**内部强制走护栏**（前置三段式提示词 + 后置四关）——未登记的 kind 必拒、缺护栏档案**启动期就失败**（`AR-33` / [ADR-0023](../background/decisions/0023-deception-content-injection.md)）。**已解耦**（[ADR-0025](../background/decisions/0025-generic-guardrailed-outlet.md)）：内核不认识「内容」，接一个新消费方 = 加 `produce`/`build` + 一条登记（[`../spec/ai-contract.md`](../spec/ai-contract.md) §6），内核零改动；「独立」由 `make archcheck` 的 `MD-4` 项保证。**已登记四个 kind**：`content` + L4 的 `intent` / `chain` / `strategy`（2026-09-21，[ADR-0031](../background/decisions/0031-analysis-reuse-and-model-backend.md)） |
...
| 诱饵资产 | 🟡 定义与多态已实现 | [`common/core/internal/decoy/`](../../common/core/internal/decoy)（`MD-25` observe-only）；**资产内容 → 边缘的执行通路未接通**（设计登记的未接通项） |
...
内容像不像、多样性够不够，要等阶段 B 把 `kind=content` 也接到模型与画像上（引入项已过许可证台账 `TB-16`）。

> 别把两件事混成一个「模型接了吗」：**L4 三步分析已接**（`--llm`，近线，不碰响应路径）；
> **欺骗内容仍未接**（离线批处理，阶段 B）。详细状态见 [`ai-capabilities.md`](ai-capabilities.md) §0 与 §3。
...
| 能力 | 状态 | 代码 | 怎么验 |
| --- | --- | --- | --- |
| 判定打分（规则求值） | ✅ | [`common/core/internal/judge/`](../../common/core/internal/judge) | `make replay` |
| 三值决策 + 灰度 | ✅ | [`common/core/internal/director/`](../../common/core/internal/director) | DAG「决策」跳 · 验证配方 |
| 会话身份 / 隔离短路 | ✅ | [`common/core/internal/session/`](../../common/core/internal/session) · [`isolation/`](../../common/core/internal/isolation) | 单测 + `doctor` ③ |
| 策略装载 / 下发 / 回执 | ✅ | [`common/core/internal/policy/`](../../common/core/internal/policy) | `make docker-log S=proxy`（已应用策略 …） |
| 事件上报 / 读侧 | ✅ | [`common/core/internal/telemetry/`](../../common/core/internal/telemetry) · `/api/events` | `scripts/shen.sh verify` |
| 存储（内存实现） | 🟡 | [`common/core/internal/store/`](../../common/core/internal/store) | 重启丢数据 —— 真实存储待接（`NI-13`） |
| 蜜罐**接入架构** | 🟡 | [`modules/honeypot/protocol/`](../../modules/honeypot/protocol) · [`common/core/internal/honeypot/`](../../common/core/internal/honeypot) | 协议注册/运行框架/最小适配器已就绪；**协议栈内容待专项调研**（用户裁定） |
| L3 网络欺骗（声明式） | 🟡 | [`modules/deception/netpolicy/`](../../modules/deception/netpolicy) | 三份 Cilium/Tetragon 模板；需集群侧加载 |
