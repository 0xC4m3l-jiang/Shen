...
1. **防缓存参数**：`decision_id` 由 `(来源, 会话, 路径, 时间窗)` 派生（`ST-10`），但**本地判定缓存键**
   2026-09-22 起还包括方法 / 查询串 / Host / UA 与策略版本（见 `modules/deception/proxy/cache.go`）。
   脚本默认追加 `?r=<随机>` ⇒ 每条请求都拿到**新判定**（不吃上一轮的分）；要复现"同输入命中缓存"
   就用 `--no-nonce`。换会话（`session: unique`）则连 `decision_id` 一起换。
