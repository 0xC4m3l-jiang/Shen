...
flowchart TB
  ...
  subgraph D["欺骗层 · Go（modules/deception 适配器 + 核心）"]
    AD["L1 适配器（这里画的是 ③ 前置 / ④ 边车：内嵌 Caddy 转发 + TLS）<br/>白名单 → 本地缓存 → 调核心 → 按三值执行 → 异步上报<br/>（另两种接入形态 ① 旁路镜像 · ② DNS 引流：前者不在请求路径上、后者是纯配置）"]
    CO["核心（无状态多副本，可随时重启）<br/>判定 judge → 决策 director（三值）<br/>会话 · 隔离 · 策略 · 遥测 · 存储 · 服务面<br/>欺骗面：诱饵 decoy · 幻境入口 honeypot · 响应生成 responder<br/>★ 判定与响应生成只在这里实现一次（AR-2 / AR-5）"]
  end

  WEB["web 真实业务层<br/>被保护的真实业务站（上游）<br/>引擎挂了它照常服务（NI-1）"]
  HON["蜜罐层<br/>幻境后端池：核心 honeypot（类型注册 + 后端池）<br/>协议仿真框架：modules/honeypot<br/>真实蜜罐：接第三方（ADR-0011）"]
  BLK["403（对手可见的处置）"]

  subgraph A["AI 模型层 · Python（analysis/）"]
    GEN["离线生成<br/>aicap 唯一出口 + 后置检查 → 内容清单（冻结字节 + checksum）"]
    NEAR["近线分析 worker<br/>读遥测 → 意图 / 攻击链 / 策略 → 结论事件"]
    MODEL["模型后端<br/>自写标准库适配器 → 云模型（DeepSeek）<br/>--llm 显式开启 · 失败回落模板并如实标注<br/>热路径永不调模型（AR-29 / AR-30）"]
  end
...
```text
攻击者 ──HTTPS──▶ 欺骗层（判定 + 三值决策 + 处置执行）
                    ├─ route_origin ─▶ web 真实业务层（原样透传）
                    ├─ route_mirage ─▶ 蜜罐层（幻境后端；改道侧再注入内容）
                    └─ block        ─▶ 403（默认关）
AI 模型层：离线生成内容 → 清单 → 策略面 → 欺骗层注入；近线读遥测出结论（不碰响应路径）
观测控制台：旁路只读（DAG / 配置快照 / 实时流）
```

> **这是功能视角的四层，不是规范分层**。规范性分层是 **`L0`–`L4` 五层**
> （[`docs/design/language.md`](docs/design/language.md) §1，`AR-1`）+ **24 个模块编号**
> （[`docs/design/modules.md`](docs/design/modules.md) §1.1 · [`docs/modules/_map.md`](docs/modules/_map.md)）。对应关系：
>
> | 本图的层 | 规范里的什么 | 注意 |
> | --- | --- | --- |
> | 欺骗层 | `L1` 边缘欺骗层 + **核心** | 核心是「欺骗层 / AI 蜜罐层」**共用**的内核（幻境入口也在核心），不是只服务欺骗层 |
> | 蜜罐层 | `L2` 协议仿真 + 核心的幻境入口与后端池 | 本项目**不实现具体蜜罐**（[`ADR-0011`](docs/background/decisions/0011-honeypot-entry-external-backends.md)） |
> | web 真实业务层 | —— | **不是我们的层**：它是被保护的上游业务（放行目标） |
> | AI 模型层 | `L4`（`analysis/`）+ 外部云模型 | `analysis/aicap` **两处都沾**：功能上服务「欺骗层」（产出欺骗内容），实现上属本层（Python） |
>
> **另有两块不在这张四层图里**：**`L3` 网络欺骗**（`modules/deception/netpolicy`，声明式微隔离 + 假拓扑）
> 与**管控平台**（`modules/console` 的只读观测台，图里单列为旁路的「观测控制台」）。
> ⚠️ 也请注意：`docs/modules/` 与 `modules/` 里那套 circled 编号（**① 欺骗层 / ② AI 蜜罐层 / ③ 管控平台**）
> 是**三个大模块**的编号，与本图这四层**不是同一套** —— 别拿本图的层号去查模块。
