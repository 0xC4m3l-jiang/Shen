...
//	② **不碰生产**：没有认证校验、没有数据库、没有真实素材（场景数据在代码里或场景包里，确定性生成）；
//	③ **失败不回落业务**：那是适配器 `deliverDecoy` 的职责（故障固定 502、绝不回生产）。
//
// 如实说明边界（`W1`–`W6` 之后仍然成立的）：
//
//	· `SHEN_WEB_LISTEN` **默认**回环（`127.0.0.1:19090`），但**不强制**——写成非回环地址也能起；
//	  隔离靠部署（验证档只在共享网络命名空间内，不发布宿主端口）；
//	· 事件出口经 `web.EventSink` → 本入口的 `eventLogger`，**写 stderr 的标准日志**；
//	  尚未接入核心遥测（见模块文档 §8 未决 4）；投递是**异步有界**的，进程退出时打印丢弃/失败计数；
//	· 只有四项启动校验（监听 / 场景包 / 配额 / 队列）+ 优雅退出（SIGINT/SIGTERM）；
//	  **没有**鉴权平台、没有场景热重载、没有动态场景发布接口。
package main
